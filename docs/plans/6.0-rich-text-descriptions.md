...
- **Use `ActionText::Content` as a parser, not a storage backend.** For plain text extraction: `ActionText::Content.new(html).to_plain_text`. This uses ActionText's parsing without its storage, sanitization, or Trix coupling.
...
A shared helper for serializers:
...
- Remove ActionText gem dependency if no remaining usage (check internal_note, Metafields::RichText)
...
- **`style` attribute in sanitizer allowlist.** Allowing arbitrary `style` attributes opens the door to CSS-based attacks (e.g., `position: fixed` overlays). May want to restrict to specific properties (color, text-align, etc.) or strip `style` entirely and rely on CSS classes.
