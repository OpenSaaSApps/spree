...
8. **Rename `Spree::Asset` → `Spree::Media` in 6.0** — aligns model/table/prefix with API naming. `Spree::Asset` kept as alias during 6.0, removed in 7.0.
...
| Field | Type | Notes |
|-------|------|-------|
| `id` | string | Prefixed ID (`asset_xxx`) |
...
- Change prefix ID from `asset_xxx` → `media_xxx`
- Keep `Spree::Asset = Spree::Media` alias for backward compat (remove in 7.0)
